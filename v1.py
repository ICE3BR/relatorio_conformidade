...
def preencher_relatorio():
    ...
    # Sequência (por COLUNA) com offsets relativos (anos, meses, dias)
    EVENTOS_SEQUENCIA = [
        ("DATA_ACAO",              0, 0, 0),
        ("DATA_PERICIA",           0, 2, 0),
    ...
    def resolver_datas(row):
        ...
        datas = {}
        reais = []
        for i, (col, _, _, _) in enumerate(EVENTOS_SEQUENCIA):
            dt = _parse_data(row.get(col))
            if dt is not None:
                reais.append((i, col, dt))

        if not reais:
            for col, _, _, _ in EVENTOS_SEQUENCIA:
                datas[col] = {'valor': '', 'prevista': False}
            return datas

        idx_anchor, col_anchor, dt_anchor = max(reais, key=lambda t: t[2])

        for i, (col, _, _, _) in enumerate(EVENTOS_SEQUENCIA[:idx_anchor + 1]):
            dt = _parse_data(row.get(col))
            datas[col] = {'valor': _fmt_dt(dt) if dt else '', 'prevista': False}

        cursor = dt_anchor
        for i in range(idx_anchor + 1, len(EVENTOS_SEQUENCIA)):
            col, anos, meses, dias = EVENTOS_SEQUENCIA[i]
    ...
    for index, row in df.iterrows():
        ...
        try:
            ...
            datas_resolvidas = resolver_datas(row)
